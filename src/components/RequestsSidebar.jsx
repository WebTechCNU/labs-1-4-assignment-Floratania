...
export default function RequestsSidebar({ isOpen, onClose, books, onRefresh }) {
  ...
  const allRequests = books.flatMap(book =>
    (book.requests || []).map(req => ({
      ...
      displayMessage: req.note || req.message || req.text || "Без повідомлення"
    }))
  ).sort((a, b) => {
              ...
              allRequests.map((req, i) => (
                <div key={i} className={`p-4 rounded-[5px] border transition-all ${req.isRead ? 'bg-slate-900/50 border-slate-800 opacity-60' : 'bg-slate-800 border-blue-500'}`}>
                  ...
                  <div className="flex justify-between items-start mb-2">
                    <span className="text-[10px] bg-blue-600 px-2 py-0.5 rounded uppercase font-bold">{req.bookTitle}</span>
                    {!req.isRead && (
                      <button onClick={() => handleMarkAsRead(req.bookId, req)} className="text-[10px] text-emerald-400 hover:bg-emerald-400 hover:text-white border border-emerald-400 px-2 py-0.5 rounded transition-colors">
                        ✓ Прочитано
                      </button>
                    )}
                  </div>
                  ...
                  <div className="mb-2">
                    <p className="font-bold text-sm">{req.userName || req.user || req.from}</p>
                  </div>

                  
                  <div className="text-xs text-slate-300 italic bg-black/20 p-3 rounded mb-4 border-l-2 border-blue-500">
                    <span className="text-slate-500 block mb-1 text-[10px] uppercase font-bold">Повідомлення:</span>
                    {req.displayMessage}
                  </div>

                  <div className="flex gap-2">
                    <a href={`tel:${req.userPhone || req.phone}`} className="flex-1 bg-slate-700 text-center py-2 rounded text-xs font-bold hover:bg-slate-600">Дзвінок</a>
                    <a href={`mailto:${req.userEmail}`} className="flex-1 bg-blue-600 text-center py-2 rounded text-xs font-bold hover:bg-blue-500">Email</a>
                  </div>
                </div>
              ))
            ) : (
              <p className="text-center text-slate-500 mt-20">Немає нових запитів</p>
            )}
          </div>
        </div>
      </div>
