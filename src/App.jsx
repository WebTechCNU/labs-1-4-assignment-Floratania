...
export default function App() {
  ...
  const [typeFilter, setTypeFilter] = useState('all'); // all, Exchange, Lend
  ...
  const confirmRequest = async (note) => {
    try {
      await api.sendRequest(requestBook._id, {
        newRequest: {
          userId: user.id, userName: user.name, userEmail: user.email, userPhone: user.phone,
          note, requestedAt: new Date().toISOString(), isRead: false
        }
      });
      setRequestBook(null); loadBooks(); alert("Запит успішно надіслано!");
